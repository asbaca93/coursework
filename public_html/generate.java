/*******************************************************************************
* Group 1
* Andrew Baca, Shane Grayson, Ian Navarro
* read.java
...
{ 
    public static void main(String[] args)throws Exception 
    {
        ...
        int numRows = 0, FileChoice = 0, idStart = 0, insertChoice = 0;
	...
	String inName = "";
        ...
        do{
		    ...
		    inName = "games";
                    break;
                case(3):
                    System.out.println("play.txt chosen to write to!");
                    Name = "play.txt";
		    inName = "play";
                    break;
                default:
	...
	do{
		System.out.println();
		System.out.println("1. Bulkload");
		System.out.println("2. Single Insertion Bulkloading");
		System.out.print("Please enter number corresponding to file choice to generate: ");
		insertChoice = scnr.nextInt();
	...
	}while(insertChoice != 1 && insertChoice != 2);
		...
		if(insertChoice == 1){
                	for(int i = 0; i <= numRows; i++){
                    		...
                    		pw.printf("%s\t", pName);             //print name to file
                    		pw.printf("%s\t", teams[randT]);      //print teams to file
		...
		if(insertChoice == 2){
			for(int i = 0; i <= numRows; i++){
				randF = rand.nextInt(40);       //randomization for names and teams
                    		...
                    		randT = rand.nextInt(32);
                    		pName = first[randF] + last[randL];     //cocatenate first and last name
				pw.printf("insert into %s values (", inName);
				pw.printf("%d, '%s', '%s', '%s', ", idStart, pName, teams[randT], pos[randP]);
				pw.print(rand.nextInt(50));
				pw.printf(", ");
				pw.print(rand.nextInt(5000));
				pw.printf(", ");
				pw.print(rand.nextInt(9000000));
				pw.println(");");
				idStart = idStart + 1;
			}
		}
		...
		if(insertChoice == 1){
                	for(int i = 0; i <= numRows; i++)
                	{
                    		...
                    		pw.print("2018"+randM+randD);          //print date to file
                    		/*note that date is ater september and day is between 10 and 30 */
                    		randS = rand.nextInt(31);
                    		pw.printf("\t%s\t", stad[randS]);       //print stadium top file
		...
		if(insertChoice == 2)
		{
			for(int i = 0; i <= numRows; i++){
				...
				randS = rand.nextInt(31);
				randR = rand.nextInt(3);
				pw.printf("'%s', '%s', ", stad[randS], result[randR]);
				pw.print(rand.nextInt(60000)+1000);
				pw.printf(", ");
				pw.printf("%d);\n", rand.nextInt(2900000)+1000000);
			}
		}
            }
		...
		if(insertChoice == 1){
                	for(int i = 0; i < numRows; i++){
                    		pw.printf("%d\t", i);      //print random ID'S
                    		pw.printf(",%d\n", numRows - i);          //print random ID's
			}
                }
		if(insertChoice == 2){
			for(int i = 0; i < numRows; i++){
